...
The JAR is versioned to match the Selenide release (e.g. `selenide-cli-7.18.2.jar`).
...
Keep `version` in lockstep with the Gradle `version` in `build.gradle` (set to `7.18.2` right before
...
node -e "console.log(require('./package.json').version)"   # e.g. 7.18.2
